...
extern struct Building *Structure_Get_ByIndex(uint16 index);
extern struct Building *Structure_Find(struct PoolFindStruct *find);

extern void Structure_Init(void);
extern void Structure_Recount(void);
extern struct Building *Structure_Allocate(uint16 index, uint8 type);
extern void Structure_Free(struct Building *s);
