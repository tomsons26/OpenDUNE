...
void GUI_Widget_TextButton_Draw(Widget *w)
{
	...
	if (w->stringID == STR_CANCEL || w->stringID == STR_PREVIOUS || w->stringID == STR_YES || w->stringID == STR_NO) {
		Fancy_Text_Print(GUI_Extract_String(w->stringID), positionX + (width / 2), positionY + 2, colour, 0, 0x122);
	} else {
		Fancy_Text_Print(GUI_Extract_String(w->stringID), positionX + 3, positionY + 2, colour, 0, 0x22);
	}
...
void GUI_Widget_SpriteTextButton_Draw(Widget *w)
{
	...
	switch (g_productionStringID) {
		...
		case STR_D_DONE:
			if (s->o.type == STRUCTURE_CONSTRUCTION_YARD) {
				...
				Draw_Shape(SCREEN_ACTIVE, g_sprites[63], positionX + 37, positionY + 5, 0, 0x100, g_paletteMapping1, buttonDown ? 2 : 0);
	...
	if (spriteID != 0) Draw_Shape(SCREEN_ACTIVE, g_sprites[spriteID], positionX + 2, positionY + 2, 0, 0x100, g_paletteMapping1, buttonDown ? 1 : 0);
	...
	if (g_productionStringID == STR_UPGRADINGD_DONE) {
		...
		Fancy_Text_Print(
			Extract_String(g_productionStringID),
			positionX + 1,
			positionY + height - 19,
			buttonDown ? 0xE : 0xF,
			0,
	...
	if (g_productionStringID == STR_D_DONE || g_productionStringID == STR_UPGRADINGD_DONE) {
		w->shortcut = GUI_Widget_GetShortcut(*Extract_String(STR_ON_HOLD));
	} else {
		w->shortcut = GUI_Widget_GetShortcut(*Extract_String(g_productionStringID));
	}
...
void GUI_Widget_TextButton2_Draw(Widget *w)
{
	...
	w->shortcut = GUI_Widget_GetShortcut(*Extract_String(stringID));
...
void GUI_Widget_ActionPanel_Draw(bool forceDraw)
{
	...
	if (actionType > 1) {
		...
		if (stringID != STR_NULL) Fancy_Text_Print(Extract_String(stringID), 288, 43, 29, 0, 0x111);
		...
		if (actionType == 2 || actionType == 3) {
			...
			Fancy_Text_Print(Extract_String(STR_DMG), 296, 65, 29, 0, 0x11);
		}
			...
			switch (actionType) {
				case 2: /* Unit */
				{
					...
					for (i = 0; i < 4; i++) {
						...
						buttons[i]->shortcut = GUI_Widget_GetShortcut(Extract_String(buttons[i]->stringID)[0]);
				...
				case 3: /* Structure */
				{
					...
					switch (o->type) {
						...
						case STRUCTURE_REPAIR: {
							...
							Fancy_Text_Print(Extract_String(STR_D_DONE), 258, 116, 29, 0, 0x11, percent);
						} break;
						...
						case STRUCTURE_STARPORT: {
							if (h->starportLinkedID != 0xFFFF) {
								Fancy_Text_Print(Extract_String(STR_FRIGATEARRIVAL_INTMINUS_D), 258, 88, 29, 0, 0x11, h->starportTimeLeft);
							} else {
								Fancy_Text_Print(Extract_String(STR_FRIGATE_INORBIT_ANDAWAITINGORDER), 258, 88, 29, 0, 0x11);
							}
						} break;
				...
				case 4: /* Attack */
					...
					Fancy_Text_Print(Extract_String(STR_SELECTTARGET), 259, 76, g_curWidgetFGColourBlink, 0, 0x11);
					break;

				case 5: /* Movement */
					GUI_Widget_MakeVisible(widget30);
					Fancy_Text_Print(Extract_String(STR_SELECTDESTINATION), 259, 76, g_curWidgetFGColourBlink, 0, 0x11);
					break;

				case 6: /* Harvest */
					GUI_Widget_MakeVisible(widget30);
					Fancy_Text_Print(Extract_String(STR_SELECTPLACE_TOHARVEST), 259, 76, g_curWidgetFGColourBlink, 0, 0x11);
					break;

				case 7: /* Placement */
					GUI_Widget_MakeVisible(widget30);
					Fancy_Text_Print(Extract_String(STR_SELECTLOCATION_TOBUILD), 259, 84, g_curWidgetFGColourBlink, 0, 0x11);
					break;
				...
				case 8: /* House Missile */
				{
					...
					Fancy_Text_Print(Extract_String(STR_PICK_TARGETTMINUS_D), 259, 84, g_curWidgetFGColourBlink, 0, 0x11, count);
				} break;
