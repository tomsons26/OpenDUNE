...
static void GUI_EditBox_BlinkCursor(uint16 positionX, bool resetBlink)
{
	...
	Hide_Mouse();
	Fill_Rect(positionX, g_curWidgetYBase, positionX + Char_Pixel_Width('W'), g_curWidgetYBase + g_curWidgetHeight - 1, (editBoxShowCursor) ? g_curWidgetFGColourBlink : g_curWidgetFGColourNormal);
	Show_Mouse();
}
	...
	*t = '\0';
	...
	for (;; sleepIdle()) {
		...
		*(++t) = '\0';
		...
		/* Draw new character */
		Fancy_Text_Print(text + textLength - 1, positionX + textWidth, g_curWidgetYBase, g_curWidgetFGColourBlink, g_curWidgetFGColourNormal, 0x020);
