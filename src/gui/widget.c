...
void GUI_Widget_Draw(Widget *w)
{
	...
	switch (drawMode) {
		...
		case DRAW_MODE_SPRITE: {
			Draw_Shape(SCREEN_ACTIVE, drawParam.sprite, offsetX, offsetY, w->parentID, 0x4100, g_remap, 1);
		} break;
		...
		case DRAW_MODE_WIRED_RECTANGLE: {
			_Draw_Rect(positionLeft, positionTop, positionRight, positionBottom, fgColour);
		} break;
...
{
	_Fill_Rect(g_curWidgetXBase << 3, g_curWidgetYBase, ((g_curWidgetXBase + g_curWidgetWidth) << 3) - 1, g_curWidgetYBase + g_curWidgetHeight - 1, g_curWidgetFGColourNormal);
}
