...
uint16 Script_Unit_DisplayDestroyedText(ScriptEngine *script)
{
	...
	if (g_config.Language == LANGUAGE_FRENCH) {
		GUI_DisplayText(Extract_String(STR_S_S_DESTROYED), 0, Extract_String(ui->o.stringID_abbrev), g_table_HouseType[Unit_GetHouseID(u)].name);
	} else {
		GUI_DisplayText(Extract_String(STR_S_S_DESTROYED), 0, g_table_HouseType[Unit_GetHouseID(u)].name, Extract_String(ui->o.stringID_abbrev));
	}
...
uint16 Script_Unit_MCVDeploy(ScriptEngine *script)
{
	...
	if (Unit_GetHouseID(u) == g_playerHouseID) {
		GUI_DisplayText(Extract_String(STR_UNIT_IS_UNABLE_TO_DEPLOY_HERE), 0);
	}
