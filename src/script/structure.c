...
uint16 Script_Structure_Destroy(ScriptEngine *script)
{
	...
	if (g_config.Language == LANGUAGE_FRENCH) {
		GUI_DisplayText("%s %s %s", 0, Extract_String(g_table_structureInfo[s->o.type].o.stringID_full), g_table_HouseType[s->o.houseID].name, Extract_String(STR_IS_DESTROYED));
	} else {
		GUI_DisplayText("%s %s %s", 0, g_table_HouseType[s->o.houseID].name, Extract_String(g_table_structureInfo[s->o.type].o.stringID_full), Extract_String(STR_IS_DESTROYED));
	}
