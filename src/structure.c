				...
				if (!s->o.flags.s.onHold && s->countDown != 0 && s->o.linkedID != 0xFF && s->state == STRUCTURE_STATE_BUSY && si->o.flags.factory) {
					...
					if (buildCost / 256 <= h->credits) {
						...
						if (buildSpeed < s->countDown) {
							...
							if (s->o.houseID == g_playerHouseID) {
								if (s->o.type != STRUCTURE_BARRACKS && s->o.type != STRUCTURE_WOR_TROOPER) {
									...
									GUI_DisplayText("%s %s", 0, Extract_String(oi->stringID_full), Extract_String(stringID));
						...
						if (s->o.houseID == g_playerHouseID) {
							...
							GUI_DisplayText(Extract_String(STR_INSUFFICIENT_FUNDS_CONSTRUCTION_IS_HALTED), 0);
						}
					}
				}
...
bool Structure_BuildObject(Structure *s, uint16 objectType)
{
			...
			if (res == FACTORY_BUY) {
				...
				for (i = 0; i < 25; i++) {
					...
					if (u == NULL) {
						...
						if (s->o.houseID != g_playerHouseID) continue;
						GUI_DisplayText(Extract_String(STR_UNABLE_TO_CREATE_MORE), 2);
						continue;
					}
	...
	if (s->o.type != STRUCTURE_CONSTRUCTION_YARD) {
		...
		str = Extract_String(g_table_unitInfo[objectType].o.stringID_full);
	} else {
		oi = &g_table_structureInfo[objectType].o;
		o = &Structure_Create(STRUCTURE_INDEX_INVALID, (uint8)objectType, s->o.houseID, 0xFFFF)->o;
		str = Extract_String(g_table_structureInfo[objectType].o.stringID_full);
	}
	...
	if (o != NULL) {
		...
		GUI_DisplayText(Extract_String(STR_PRODUCTION_OF_S_HAS_STARTED), 2, str);
...
bool Structure_SetUpgradingState(Structure *s, int8 state, Widget *w)
{
	...
	if (state == 0 && s->o.flags.s.upgrading) {
		if (s->o.houseID == g_playerHouseID) {
			GUI_DisplayText(Extract_String(STR_UPGRADING_STOPS), 2);
		}
	...
	if (s->o.houseID == g_playerHouseID) {
		GUI_DisplayText(Extract_String(STR_UPGRADING_STARTS), 2);
	}
...
bool Structure_SetRepairingState(Structure *s, int8 state, Widget *w)
{
	...
	if (state == 0 && s->o.flags.s.repairing) {
		if (s->o.houseID == g_playerHouseID) {
			GUI_DisplayText(Extract_String(STR_REPAIRING_STOPS), 2);
		}
	...
	if (s->o.houseID == g_playerHouseID) {
		GUI_DisplayText(Extract_String(STR_REPAIRING_STARTS), 2);
	}
